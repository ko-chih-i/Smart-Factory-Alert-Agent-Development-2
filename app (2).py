...
def main():
    ...
    st.caption("即時感測器 telemetry 時序分析 (Plotly) | 孤立森林 ML | Gemini 維修建議")
    ...
    m2.metric("正常狀態", f"{total_count - abnormal_count} 筆")
    m3.metric("異常警報", f"{abnormal_count} 筆", delta=f"{abnormal_count/total_count*100:.1f}%", delta_color="inverse")
    m4.metric("緊急警報", f"{len(processed_df[processed_df['severity']=='CRITICAL'])} 筆", delta_color="inverse")
